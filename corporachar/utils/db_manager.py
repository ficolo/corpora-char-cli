...
CREATE_PROCESS_LOG = """
    CREATE TABLE IF NOT EXISTS Process_Log(id_log MEDIUMINT NOT NULL AUTO_INCREMENT, file_name TEXT,
     error TEXT, exception TEXT, PRIMARY KEY (id_log))
    """
CREATE_ANNOTATIONS_STMT = """
    ...
    bio_class_id TEXT, bio_ontology_id TEXT, text TEXT, match_type TEXT, frequency INT, PRIMARY KEY (id_annotation))
    """
...
INSERT_LOG = """
    INSERT INTO Process_Log(file_name, error, exception) VALUES ('{}', '{}', '{}')
    """
...
class DBConnect:
        ...
        try:
            ...
            for class_id in annotations:
                insert = INSERT_ANNOTATION.format(
                    ...
                    annotations[class_id]['bio_class_id'],
                    annotations[class_id]['bio_ontology_id'],
                    pymysql.escape_string(annotations[class_id]['text']),
    ...
    def insert_log(self, log):
        try:
            cur = self.con.cursor()
            insert = INSERT_LOG.format(log['file_name'], log['error'], log['exception'])
            cur.execute(insert)
            self.con.commit()
        except (pymysql.Error, UnicodeEncodeError) as e:
            print "Error %s:" % e.args[0]
